...
let AppService = class AppService {
    constructor() {
        this.config = (0, shared_config_1.readRuntimeConfig)('resource-hr');
        this.capabilities = (0, security_profile_1.getSecurityCapabilities)(this.config.securityProfile);
        this.logger = new shared_logging_1.StructuredLogger('resource-hr');
    ...
    async createResource(body, request) {
        const resource = {
            ...
            service: this.config.serviceName,
            department: this.serviceDescriptor.department || 'hr',
...
async function bootstrap() {
    const config = (0, shared_config_1.readRuntimeConfig)('resource-hr');
    const app = await core_1.NestFactory.create(app_module_1.AppModule);
    const globalPrefix = 'api';
    app.setGlobalPrefix(globalPrefix);
    await app.listen(config.port);
    common_1.Logger.log(`resource-hr listening on http://localhost:${config.port}/${globalPrefix}`);
}
bootstrap();
